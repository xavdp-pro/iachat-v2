      ...
      {expanded && (
        <div style={{ padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            {row.serrure?.ref && (
              <Cell icon={<Shield size={11} />} label="Serrure" value={`${row.serrure.ref}${row.serrure.mode ? ` (${row.serrure.mode})` : ''}`} />
            )}
            {row.ferme_porte?.ref && (
              <Cell icon={<Wrench size={11} />} label="Ferme-porte" value={`${row.ferme_porte.ref}${row.ferme_porte.prix != null ? ` — ${row.ferme_porte.prix} € HT` : ''}`} />
            )}
            {row.accessoires?.length > 0 && (
              <Cell icon={<Wrench size={11} />} label="Accessoires" value={row.accessoires.join(' · ')} />
            )}
            {row.equip_extra?.length > 0 && (
              <div style={{ fontSize: '11px', color: 'var(--color-text-2)', display: 'flex', flexDirection: 'column', gap: '2px', padding: '4px 2px' }}>
                {row.equip_extra.map((e, i) => <span key={i}>🔧 {e}</span>)}
              </div>
            )}
            {row.autres && (
              <div style={{ fontSize: '11px', color: 'var(--color-text-2)', padding: '4px 2px' }}>📎 {row.autres}</div>
            )}
            {row.docs?.length > 0 && (
              <div style={{ fontSize: '10px', color: 'var(--color-text-3)', fontStyle: 'italic', padding: '4px 2px' }}>
                📄 {row.docs.join(' → ')}
              </div>
            )}
          </div>
...
// ══════════════════════════════════════════════════════════════════════════════
export default function ProspectQuotes() {
  ...
  useEffect(() => {
    if (!id) return
    api.get(`/prospects/companies/${id}`)
      .then(data => {
        setCompanyName(data?.company?.properties?.name || `Entreprise #${id}`)
      })
      .catch(() => {
        setCompanyName(`Entreprise #${id}`)
      })
  }, [id])

  // Sessions = historique de fichiers analysés (localStorage)
  const [sessions, setSessions] = useState(() => {
    try { return JSON.parse(localStorage.getItem(`devis_sessions_${id}`) || '[]') } catch { return [] }
  })

  // Synchronize localStorage when sessions change
  useEffect(() => {
    try {
  ...
  const handleCreateQuote = async () => {
    if (!newQuoteName.trim()) {
      setError('Veuillez donner un nom au devis')
      return
    }
    if (!pendingFile) {
    ...
    await analyzeFile(pendingFile, newQuoteName.trim())
  }

  // ── Analyse ────────────────────────────────────────────────────────────
  const analyzeFile = async (f, customName = null) => {
    setAnalyzing(true)
    setError('')
    try {
  ...
  const leftPanelProps = {
    ...
    onFileChange: (f) => {
      setPendingFile(f)
      if (!createModalOpen) {
        setNewQuoteName(f.name.replace(/\.xlsx$/i, ''))
        setCreateModalOpen(true)
      }
    },
  }
  ...
  const centerColumn = (
    <div style={{ width: 450, minWidth: 450, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: assistantDock === 'right' ? '1px solid var(--color-border)' : 'none' }}>
      {analyzing ? (
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--color-bg)', color: 'var(--color-text)', fontFamily: 'var(--font-body)' }}>
      ...
      <header className="admin-topbar" style={{ borderRadius: 0, flexShrink: 0, margin: 0 }}>
        ...
        <div className="admin-topbar-actions">
          ...
          <button type="button" className="admin-btn-ghost" onClick={() => navigate('/prospects')}>
            <ChevronLeft size={16} />
      ...
      >
        {assistantDock === 'bottom' ? (
          ...
          <>
            ...
            {centerColumn}
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              <RightPanel {...rightPanelProps} layoutMode="right" panelWidth="100%" />
            </div>
          </>
        )}
      </div>
      ...
      {/* ── Modal Creation Devis ── */}
      {createModalOpen && (
        <div className="chat-modal-backdrop" onClick={() => setCreateModalOpen(false)}>
          <div
            className="chat-modal"
            style={{ maxWidth: 460, padding: '1.25rem' }}
            onClick={e => e.stopPropagation()}
          >
            <div className="chat-modal-header" style={{ marginBottom: '1.25rem', padding: 0 }}>
              <h2 className="chat-modal-title">
                <Plus size={18} color="var(--color-primary)" style={{ marginRight: 8, verticalAlign: 'middle' }} />
                Nouveau Devis
              </h2>
              <button className="chat-modal-close" onClick={() => setCreateModalOpen(false)} style={{ top: 0, right: 0 }}><X size={16} /></button>
            </div>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', color: 'var(--color-text-3)', marginBottom: '0.5rem', fontWeight: 600, textTransform: 'uppercase' }}>
                  Nom du devis
                </label>
                <input
                  className="chat-modal-input"
                  style={{ width: '100%' }}
                  placeholder="Ex: Devis Portes EI30..."
                  value={newQuoteName}
              ...
              <div>
                ...
                <div
                  ...
                  style={{
                    padding: '1.5rem',
                    textAlign: 'center',
                    border: `2px dashed ${pendingFile ? 'var(--color-primary)' : 'var(--color-border)'}`,
                    borderRadius: '10px',
                    cursor: 'pointer',
                    background: pendingFile ? 'rgba(var(--color-primary-rgb), 0.05)' : 'rgba(255,255,255,0.02)',
                    transition: 'all 0.2s'
                  }}
                >
                  <Upload size={24} style={{ opacity: 0.5, marginBottom: 8, color: pendingFile ? 'var(--color-primary)' : 'inherit' }} />
                  <div style={{ fontSize: '0.8125rem', fontWeight: 500 }}>
                    {pendingFile ? pendingFile.name : 'Cliquer pour sélectionner le fichier'}
                  </div>
                  {!pendingFile && <div style={{ fontSize: '0.75rem', color: 'var(--color-text-3)', marginTop: 4 }}>Format Excel attendu</div>}
                </div>
              </div>

              {error && (
                <div style={{ fontSize: '0.75rem', color: '#ef4444', background: 'rgba(239,68,68,0.1)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.2)' }}>
                  ⚠️ {error}
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
                ...
                <button 
                  ...
                  disabled={analyzing}
                >
                  {analyzing ? <Loader2 size={16} className="animate-spin" /> : 'Créer et analyser'}
                </button>
              </div>
            </div>
          </div>
