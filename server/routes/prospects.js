...
// GET /api/prospects/owners?after=&limit= — HubSpot owners usable as deal owners
router.get('/owners', async (req, res) => {
  try {
    ...
    const data = await listOwners({ after, limit })
    res.json(data)
  } catch (err) {
    if (err.code === 'NO_TOKEN') {
      return res.status(503).json({ error: err.message })
    }
    console.error('[prospects] GET /owners', err)
...
router.post('/companies/:id/deals', async (req, res) => {
  try {
    ...
    const [[creator]] = await db.query(
      'SELECT hubspot_user_id, hubspot_user_email, hubspot_user_name FROM users WHERE id = ?',
      [req.user.id]
    )
    const deal = await createDealForCompany({
      ...
      pipeline,
      dealstage,
      ownerId: creator?.hubspot_user_id || null,
    })
